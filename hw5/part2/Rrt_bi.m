...
% find a path using an RRT bi-directional algorithm
%% initialization
clear ; close all; clc
...
rawdata = dlmread('world_obstacles.txt');
...
% find the path
[path, path2] = Find_path_bi(obj,obs,param,start,goal);
...
% plot the path2
for i=2:length(path2)
    plot([path2(i,1);path2(i-1,1)],[path2(i,2);path2(i-1,2)],'g','LineWidth',1.5);
end
